...
function isoWeekdaySun0(iso_date)
{
    ...
    if (!isISODate(iso_date)) { return null; }
...
function timestampOfDateTime(iso_date, hhmm)
{
    ...
    if (!iso_date || !hhmm) { return null; }

    if (!isISODate(iso_date)) { return null; }

    date_parts = iso_date.split('-');
    time_parts = hhmm.split(':');
    if (date_parts.length != 3 || time_parts.length != 2) { return null; }
...
function calibrateAnchor(x1, y1, ringRestHeight)
{
    ...
    if (x1 == null || y1 == null || ringRestHeight == null) { return null; }

    if (isNaN(x1) || isNaN(y1) || isNaN(ringRestHeight)) { return null; }
...
function stateFromBackupText(text)
{
...
{
    if (window.workout_sample_workout)
    {
        return window.workout_sample_workout;
    }
...
function setWorkoutDate(workout, iso_date)
{
    ...
    if (!isISODate(iso_date)) { return false; }
...
function adjacentLoggedDayWorkout(workout, direction)
{
    ...
    for (i = 0; i < state.workouts.length; ++i)
    {
        ...
        if (!candidate || !isISODate(candidate.date) || candidate.date == workout.date) { continue; }
    ...
    if (target_date == null) { return null; }
...
function rebuildLadderSets(ex, ladder_rungs)
{
    ...
    for (i = 0; i < count; ++i)
    {
        ...
        if (!done.length && !rungs.length) { continue; }
...
function loadSample()
{
    ...
    if (!sample || !sample.exercises || !sample.exercises.length) { return; }
...
function moveExercise(workout, exId, delta)
{
    ...
    for (i = 0; i < workout.exercises.length; ++i)
    {
        if (workout.exercises[i].id != exId) { continue; }

        j = i + delta;
        if (j < 0 || j >= workout.exercises.length) { return; }
